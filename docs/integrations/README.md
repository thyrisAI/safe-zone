...
| agentgateway | Level 2 preview | Buffered native attachment complete | 1.5.0 standalone; 1.5 policy schema | OpenAI Chat Completions, Responses, Anthropic Messages, MCP, A2A, and generic JSON HTTP content | Buffered request/response mask, block, audit-only | Not yet claimed | `TSZGuardrailPolicy` to controller-owned `AgentgatewayPolicy` |

...
| agentgateway | Compatibility track | Phase 2 in progress | 1.5 policy schema; live matrix pending | OpenAI Chat Completions, Responses, Anthropic Messages, MCP and A2A JSON-RPC content | Buffered request/response mask, block, audit-only | Not yet claimed | Manual `AgentgatewayPolicy`; controller reconciliation pending |
